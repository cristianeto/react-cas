...
import { Grid, Paper, Divider, Container, Typography } from '@material-ui/core';
...
class UserAddForm extends Form {
  ...
  schema = Joi.object({
    id: Joi.string(),
    belongs_espoch: Joi.string()
      .label('Usuario ESPOCH')
      .max(2)
      .messages(messages),
  ...
  populateCheckedPermissions() {
    ...
    permissions.map((role) =>
      permissionsUser.filter((r) => r.id === role.id).length > 0
        ? (role.isChecked = true)
  ...
  handleSearch = async () => {
    ...
    try {
      const { data: espochUser } = await getEspochUser(
        this.state.data.identification_card
      );
      if (Object.keys(espochUser).length) {
        const data = { ...this.state.data };
        data['name'] = espochUser.per_nombres;
        data['lastname'] =
          espochUser.per_primerApellido + ' ' + espochUser.per_segundoApellido;
  ...
  doUpdateRoles = async (e) => {
    ...
    const arrayRolesToSave = rolesChecked.filter(
  ...
  doUpdatePermissions = async (e) => {
    ...
    const arrayPermissionsToSave = permissionsChecked.filter(
      (permissions) => permissions.isChecked === true
    );
    const permissionsIds = arrayPermissionsToSave.map(
  ...
  doSubmit = async () => {
    try {
      const data = { ...this.state.data };
      data['belongs_espoch'] === 'Sí'
        ? (data['belongs_espoch'] = true)
        : (data['belongs_espoch'] = false);
      await saveUser(data);
      this.successMessage();
