...
function AddRequirementForm({activities, populateRequirements}) {
  ...
  const handleChange = ({ target: input }) => {
    ...
    if (input.name !=='name') data['total_price']= data['quantity']*data['price']
  };
  ...
  const schema = Joi.object({
    name: Joi.string().label("Descripción").min(3).max(500).messages(messages),
    quantity: Joi.number().label("Cantidad").positive().integer().messages(messages),
    price: Joi.number().precision(2).label("Precio unitario").positive().messages(messages),
    total_price: Joi.number().precision(2).label("Precio total").messages(messages),
  })
  ...
  return (
    <div>
      ...
      <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title">
        <DialogTitle id="form-dialog-title">Datos del requerimiento</DialogTitle>
        <DialogContent>
          <TextField
