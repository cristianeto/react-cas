...
class ProjectForm extends Form {
  ...
  getStepContent = (step) => {
    ...
    switch (step) {
        ...
        return (
          <React.Fragment>
            <SwitchLabel
              openLabel='Abierto'
              closeLabel='Cerrado'
              checked={this.state.data.is_open}
      ...
      case 2:
        return <React.Fragment>3er Componente</React.Fragment>;
      case 3:
  ...
  schema = Joi.object({
    is_open: Joi.boolean().messages(messages),
    id: Joi.string().guid(),
    ...
    project_type_id: Joi.number()
      ...
      .label('Tipo proyecto')
      .messages(messages),
    coverage_type_id: Joi.number()
      .integer()
      .less(7)
      .label('Tipo cobertura')
      .messages(messages),
    research_type_id: Joi.number()
      .integer()
      .less(4)
      .label('Tipo investigación')
      .messages(messages),
    ...
    current_situation: Joi.string()
      ...
      .max(500)
      .messages(messages),
    general_objective: Joi.string()
      .label('Objetivo General')
      .allow('')
      .max(500)
      .messages(messages),
    specific_objectives: Joi.string()
    ...
    expected_results: Joi.string()
      ...
      .max(500)
      .messages(messages),
    transference_results: Joi.string()
      .label('Transferencia de resultados')
      .allow('')
      .max(500)
      .messages(messages),
    ...
    impacts: Joi.string()
      ...
      .messages(messages),
    aspects: Joi.string()
      .label('Aspectos bioéticos y sociales')
      .allow('')
      .max(500)
      .messages(messages),
  });
  ...
  mapToViewModel(project) {
    return {
      is_open: project.is_open === 0 ? false : true,
      id: project.id,
      name: project.name,
      slug: project.slug,
      ...
      project_type_id:
        project.project_type_id === null ? '' : project.project_type_id,
      research_type_id:
        project.research_type_id === null ? '' : project.research_type_id,
      coverage_type_id:
        project.coverage_type_id === null ? '' : project.coverage_type_id,
      ...
      justification:
        project.justification === null ? '' : project.justification,
      current_situation:
        project.current_situation === null ? '' : project.current_situation,
      general_objective:
        project.general_objective === null ? '' : project.general_objective,
      specific_objectives:
        project.specific_objectives === null ? '' : project.specific_objectives,
      sustainability:
        project.sustainability === null ? '' : project.sustainability,
      methodology: project.methodology === null ? '' : project.methodology,
      expected_results:
        project.expected_results === null ? '' : project.expected_results,
      transference_results:
        project.transference_results === null
          ? ''
          : project.transference_results,
      beneficiaries:
        project.beneficiaries === null ? '' : project.beneficiaries,
  ...
  handleChangeSwitch = async (event) => {
    const data = { ...this.state.data };
    data.is_open = event.target.checked;
    this.setState({ data });
    try {
      await updateProject_isOpenField(this.state.data.slug);
