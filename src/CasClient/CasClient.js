...
export class CasClient {
    ...
    validateLogin() {
        ...
        var urlvalidate = CONFIG2.CASVALIDATE + 'service=' + service + '&ticket=' + ticket
        console.log(urlvalidate);
        alert(urlvalidate);
        let promise = new Promise((resolve, reject) => {
            //  this.http.doGetUrlXML(urlvalidate).subscribe(res => this.validation(resolve, reject, res), reject)
            this.http.doGetUrlXML(urlvalidate).subscribe((res) => { console.log(res); this.validation(resolve, reject, res) }, err => {
    ...
    validation(resolve, reject, res) {
        let Informacion = x2js.parseString(res, function (err, resultado) {
            ...
            if (sucesso !== undefined) {
                var sucess = resultado['cas:serviceResponse']['cas:authenticationSuccess'];
